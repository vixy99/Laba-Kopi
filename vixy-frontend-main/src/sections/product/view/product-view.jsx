...
const ProductPage = () => {
  ...
  const handleActivateRow = async (id) => {
    await instance.post(`Products/activation/${id}`).then(() => {
      getData();
    }).catch(err => console.log(err))
  };

  const handleEdit = (id) => navigate(`/dashboard/admin/product/${id}/edit`)
  ...
  return (
    <Container>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
        <Typography variant="h4">Data Product</Typography>

        <Button component={RouterLink} to='/dashboard/admin/product/create' variant="contained" color="inherit" startIcon={<Iconify icon="eva:plus-fill" />}>
          New Product
        </Button>
      </Stack>
