...
const UnitPage = () => {
  ...
  const getData = async () => {
    await instance.get('units').then(({data}) => {
      setListData(data);
  ...
  const handleActivateRow = async (id) => {
    await instance.post(`units/activation/${id}`).then(() => {
      getData();
    }).catch(err => console.log(err))
  };

  const handleEdit = (id) => navigate(`/dashboard/admin/unit/${id}/edit`)
  ...
  return (
    <Container>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
        ...
        <Button component={RouterLink} to='/dashboard/admin/unit/create' variant="contained" color="inherit" startIcon={<Iconify icon="eva:plus-fill" />}>
          New Unit
        </Button>
      </Stack>
